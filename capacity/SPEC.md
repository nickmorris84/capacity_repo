...
§26.6 Global presets (landing) — Session B. The ONLY place presets are created or edited: Channel presets (four built-ins + user-created), Arrival patterns, Seasonality — full create/edit/delete, available to every simulation. All in-simulation preset creation/editing affordances are REMOVED; workspace pickers read the global library only. Apply semantics remain copy-on-apply with provenance {presetName, appliedAt} and a per-use "Re-sync from global" (diff preview, then apply) — landing edits never silently mutate a simulation or its Runs. (In Session A the libraries remain temporarily reachable inside Simulation Settings so nothing is orphaned; B relocates them.)

§26.7 Compare (landing) — Session B. Verdict-first comparator picker (2–4 comparators, Runs plus a "Live now" entry per simulation, persistent colour chips, one switchable reference): headline strip with auto-verdict, "What changed" config diff vs the reference, collapsible Money / Service / People sections with direction-coloured delta chips, and a per-queue accordion sorted by largest absolute delta. Live comparators compute on demand and cache until updatedAt changes; Runs render instantly. Nothing scrolls the page horizontally.

### §26 gate tests (Session A)
Migration: seed old-schema storage → open → "Simulation 1" present with snapshots intact as Runs. JSDOM: the landing renders; create via (b) duplicates everything except runs; wizard (c) end-to-end produces a simulating workspace; rename persists; delete requires typed confirmation and removes only its record; back-navigation round-trips without state loss; the workspace has no New-simulation menu and no Runs tab, and Save creates a named Run; Simulation Settings shows collapsible sections with Start first containing HC start + week-1 date. `/dist` rebuilt.
