...
| **v1.4** | **Shared capacity is a queue type** | **service teams are retired as a separate concept; shared capacity is a queue of type `shared_capacity`.** Several processes routing through one such queue is how a single pool serves many journeys — the job service teams did. It reaches the engine as `resourcing: "leveraged"`, which is the representation the engine itself moved to (§21 already converted legacy service teams into exactly this). Any `serviceTeams` already in a carried `engineConfig` keep simulating untouched; they are simply no longer authored |
