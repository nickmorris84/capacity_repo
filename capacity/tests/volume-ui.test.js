...
await t("mounts; the grid shows the full spine with sum/entered/equal provenance", () => {
  ...
  eq(val("Voice"), "2398", "single channel inherits the full figure");
...
await t("THE OWNER'S WORKED EXAMPLE: 10,000 at brand + 5,000 at Collections", () => {
  ...
  const chans = $$(".volrow:not(.head)", c).filter((r) => ["Voice", "Digital"].includes($(".volname", r).textContent));
  const newCardChans = chans.slice(-2); // the last rt's two channels
